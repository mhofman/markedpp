 ...
 * @return {String}
 */
Parser.prototype.headingAutoId = function (token) {
	var id = '';
	if (false /*github*/) {
		// TODO - 
...
Parser.prototype.updateAutoIdentifier = function () {
	...
	function prep (id) {
		id = id.replace(/^#/, '').replace(/(?:%20|\+)/g, ' ');
		id = '#' + self.headingAutoId({text: id});
		return id;
	}

	/* TODO -- Removed
	function strip (inline, number) {
		var text = '';
		if (number) {
			text += number + ' ';
		}
		inline.forEach(function(token){
			text += token.label || token.text || '';
		});
		return text.trim();
	}
	*/
